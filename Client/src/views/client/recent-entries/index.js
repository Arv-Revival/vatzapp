...
const RecentEntries = (props) => {
	...
	const setWidth = (minWidth) => {
		...
		let dynamicWidth = gridWidth - 912;
	...
	const getData = () => {
		setShowLoader(true);
		callApi("get", ApiConstants.entry.clientrecentlist, {}, true)
			.then((response) => {
				setShowLoader(false);
						...
						<Card.Body className="p-4">
							...
							{windowWidth && (
								<Grid data={gridData} skip={gridState.skip} pageSize={gridState.take} pageable={pagerSettings} onPageChange={pageChange} filterable={true} filter={gridState.filter} onFilterChange={filterChange}>
									<Column field="id" title="#" width={60} filterable={false} cell={(props) => <td>{props.dataIndex + 1}</td>} />
									<Column
										field="invoice_date"
										width={setWidth(180)}
										filterable={false}
									...
									/>
									<Column field="invoice_number" title="Invoice #" width={180} />
									...
									<Column
										...
										cell={(props) => (
											<td>
												{(props.dataItem.entry_status_id === entryStatus.INPROGRESS || props.dataItem.entry_status_id === entryStatus.RECHECK) && <div className="text-warning">In Progress</div>}
													...
													{props.dataItem.entry_status_id !== entryStatus.APPROVED && (
														<div className="action-panel">
															<button
																...
																{props.dataItem.requested_for_delete ? "Requested" : "Request for Delete"}
															</button>
														</div>
													)}
													&nbsp;
