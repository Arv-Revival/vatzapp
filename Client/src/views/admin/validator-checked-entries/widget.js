...
import { Row, Col, Modal, Accordion, Card, ListGroup } from "react-bootstrap";
...
const CheckedEntriesWidget = (props) => {
  ...
  const getData = React.useCallback(() => {
    setShowLoader(true);
    callApi("get", ApiConstants.admin.adminvalidatorchekedlist, { row_count: props.row_count }, true)
      .then((response) => {
        setShowLoader(false);
        ...
        <Row md={0}>
          <Col xl={12}>
            <Grid data={entriesList} style={{ height: 270 }}>
              ...
              <Column field="name" title="Client Name" width="220px" />
                ...
                <Card key={row.id}>
                  <Accordion.Toggle style={{ backgroundColor: "#7599b1" }} as={Card.Header} eventKey={row.id}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      {row.name}
                      {/* <Button variant="outline-light" size="sm"><FaAngleDown/></Button> */}
                    </div>
                  </Accordion.Toggle>
