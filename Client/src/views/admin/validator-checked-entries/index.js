...
import { Row, Col, Card, Modal, Button, Accordion, ListGroup, InputGroup, FormControl } from "react-bootstrap";
...
const CheckedEntries = (props) => {
  ...
  const onSearchFilter = () => {
    ...
    if (windowWidth) {
      ...
      if (endDate && startDate) {
        console.log(endDate);
        setAccordionList(
          accordionList.filter((data) => {
            if (new Date(data.invoice_date) >= new Date(startDate) && new Date(data.invoice_date) <= new Date(endDate)) console.log(data);

            return new Date(data.invoice_date) >= new Date(startDate) && new Date(data.invoice_date) <= new Date(endDate);
          })
        );
      }
    }
  ...
  const getData = () => {
    setShowLoader(true);
    callApi("get", ApiConstants.admin.adminvalidatorchekedlist, {}, true)
      .then((response) => {
        setShowLoader(false);
          ...
          <Card className="rounded">
            <Card.Body className="p-4">
              <Row className="mb-3">
                ...
                <Col xs={10} md={1} xl={1}>
                  <button className="btn-icon btn btn-primary search-button" onClick={onSearchFilter}>
                            ...
                            <Accordion.Collapse eventKey={row.id}>
                              <Card.Body>
                                <div className="action-panel" style={{ dispaly: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
                                  <button className="btn btn-outline-primary" title="View" onClick={() => viewEntry(row)}>
                                    View <i className="feather icon-eye"></i>
                                  </button>
                                </div>
                                <ListGroup>
                                  <ListGroup.Item>
                                    <span style={{ padding: "0 8px 0 8px" }}>Validator:</span>
                                    <span> {row.validator_name}</span>
                                  </ListGroup.Item>
                                  <ListGroup.Item>
                                    <span style={{ padding: "0 8px 0 8px" }}>Checker:</span>
                                    <span> {row.checker_name}</span>
                                  </ListGroup.Item>
                                  <ListGroup.Item>
                                    <span style={{ padding: "0 8px 0 8px" }}>Invoice Date: </span>
                                    <span> {moment(row.invoice_date).format("DD-MMM-YYYY")}</span>
                                  </ListGroup.Item>
                                  <ListGroup.Item>
                                    <span style={{ padding: "0 8px 0 8px" }}>Invoice Number: </span>
                                    <span> {row.invoice_number}</span>
                                  </ListGroup.Item>
                                  <ListGroup.Item>
                                    <span style={{ padding: "0 8px 0 8px" }}>Amount: </span>
                                    <span> {row.amount}</span>
                                  </ListGroup.Item>

                                  <ListGroup.Item style={{ display: "flex" }}>
                                    <span style={{ padding: "0 8px 0 8px" }}>Type:</span>
