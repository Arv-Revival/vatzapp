...
const ApprovedEntriesWidget = (props) => {
  ...
  const getData = React.useCallback(() => {
    setShowLoader(true);
    callApi("get", ApiConstants.entry.checkerapprovedlist, { row_count: props.row_count }, true)
      .then((response) => {
        setShowLoader(false);
        ...
        <Row>
          <Col xl={12}>
            <Grid data={entriesList} style={{ height: 270 }}>
              ...
              <Column field="invoice_date" filterable={false} width="130" title="Invoice Date" cell={(props) => <td>{moment(props.dataItem.invoice_date).format("DD-MMM-YYYY")}</td>} />
              <Column field="invoice_number" title="Invoice #" width="110px" />
              <Column field="amount" title="Amount" width="100px" />
              <Column field="entry_type" title="Type" width="110px" cell={(props) => <td>{props.dataItem.entry_type === entryTypes.SALE ? "Sales" : props.dataItem.entry_type === entryTypes.PURCHASE ? "Purchase" : props.dataItem.entry_type === entryTypes.EXPENDITURE ? "Expenditure" : ""}</td>} />
              <Column
                field="View"
                title="View"
                width="80px"
...
export default ApprovedEntriesWidget;
