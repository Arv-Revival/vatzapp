...
const CheckerUsers = (props) => {
  ...
  const getUsers = () => {
    setShowLoader(true);
    callApi("get", ApiConstants.checker.list, {})
      .then((response) => {
        setShowLoader(false);
                ...
                <Col xs={12}>
                  {windowWidth && (
                    <Grid data={userGridData} skip={usersState.skip} pageSize={usersState.take} pageable={pagerSettings} onPageChange={pageChange} filterable={true} filter={usersState.filter} onFilterChange={filterChange}>
                      ...
                      <Column field="email" width="280" title="Email" />
                      <Column
                        field="whatsapp_no"
                        title="Whatsapp No"
                        width="200px"
                        cell={(props) => (
                          <td>
                            <div>{props.dataItem.w_country_code + " " + props.dataItem.whatsapp_no}</div>
                          </td>
                        )}
                      />
                      <Column field="validator_name" width="150" title="Validator" />
                      <Column field="clients_count" width="150" filterable={false} title="Number of clients" cell={(props) => <td>{props.dataItem.clients_count ? props.dataItem.clients_count : 0}</td>} />
                      <Column
                        field="join_date"
                        width="150"
                        filterable={false}
                      ...
                      <Column
                        ...
                        cell={(props) => (
                          <td>
                            <div className={props.dataItem.is_active ? "text-success" : "text-danger"}>{props.dataItem.status}</div>
                          </td>
                        )}
                      />
                      <Column
                        ...
                        <InputGroup className="mb-2">
                          <FormControl
                            ...
                            onChange={(event) => {
                              ...
                              return event.target.value ? setAccordionList(usersList?.filter((data) => data.name?.includes(event.target.value))) : setAccordionList(usersList);
                            }}
                          />
                        </InputGroup>
                        <InputGroup className="mb-2">
                                ...
                                <Accordion.Collapse eventKey={row.id}>
                                  <Card.Body>
                                    ...
                                    <ListGroup>
                                      ...
                                      <ListGroup.Item>
                                        <span style={{ padding: "0 16px 0 8px" }}>Number of Client:</span>
                                      ...
                                      <ListGroup.Item>
                                        ...
                                        <span className={row.is_active ? "text-success" : "text-danger"}>{row.status}</span>
                                      </ListGroup.Item>
                                    </ListGroup>
                                  </Card.Body>
                                </Accordion.Collapse>
